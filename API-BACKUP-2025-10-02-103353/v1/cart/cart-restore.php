...
switch ($action) {
    case 'add':
        ...
        // For Polished color, try to find the polished image
        // Most polished variants have 'P' appended to the filename
        if ($color === 'Polished' && !empty($product['ImageFile'])) {
            // Check if there's a polished variant image
            $base_name = pathinfo($product['ImageFile'], PATHINFO_FILENAME);
            $extension = pathinfo($product['ImageFile'], PATHINFO_EXTENSION);
            $polished_image = $base_name . 'P.' . $extension;
            // Use polished image (this is a simple approach - ideally check if file exists)
            $image_url = 'https://dentwizard.lgstore.com/pdf/244/' . $polished_image;
        }
