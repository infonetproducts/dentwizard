...
switch ($action) {
    case 'add':
        ...
        // Get color variant image if color is specified
        $image_url = $product['ImageFile'] ? 'https://dentwizard.lgstore.com/pdf/' . $product['CID'] . '/' . $product['ImageFile'] : '';
        
        if (!empty($color)) {
            // Get color-specific image using the same logic as detail.php
            $color_query = "SELECT color_image 
    ...
    case 'get':
    default:
        // Get cart - no changes needed here
        break;
