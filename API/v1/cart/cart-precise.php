...
switch ($action) {
    case 'add':
        ...
        $image_url = !empty($product['ImageFile']) ? 'https://dentwizard.lgstore.com/pdf/244/' . $product['ImageFile'] : '';
        
        // If color is specified, get the color-specific image EXACTLY like detail.php does
        if (!empty($color)) {
            // Use the EXACT same query as detail.php
            $color_sql = "SELECT color_image 
                          FROM item_group_options 
                          WHERE item_id = $product_id 
                          AND CID = 244 
                          AND display_name = '" . $mysqli->real_escape_string($color) . "'
                          AND price = 0
                          LIMIT 1";
            ...
            if ($color_result && $color_result->num_rows > 0) {
                ...
                if (!empty($color_row['color_image'])) {
                    // Build URL EXACTLY like detail.php does
                    $image_url = 'https://dentwizard.lgstore.com/pdf/244/' . $color_row['color_image'];
                }
            }
        }
        ...
        if (!$found) {
            $_SESSION['cart_items'][] = array(
                ...
                'size' => $size,
                'color' => $color,
                'image' => $image_url  // This will have the correct color variant image
            );
        }
...
// Add budget info (required by React app)
