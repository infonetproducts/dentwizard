...
switch ($action) {
    case 'add':
        ...
        // Try to get color-specific image - SAFELY with fallback
        if (!empty($color)) {
            try {
                // Same query that works in detail.php
                ...
                if ($color_result && $color_result->num_rows > 0) {
                    ...
                    if (!empty($color_row['color_image'])) {
                        // Use the color-specific image
                        $image_url = 'https://dentwizard.lgstore.com/pdf/244/' . $color_row['color_image'];
                    }
                }
            } catch (Exception $e) {
                // If color query fails, just use default image (don't break cart)
            }
        }
