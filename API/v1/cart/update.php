<?php
// Cart Update Quantity API - PHP 5.6 Compatible
...
if ($new_quantity === 0) {
    ...
    foreach ($_SESSION['cart'] as $key => $item) {
        if ($item['id'] == $item_id) {
            ...
            $_SESSION['cart'][$key]['total'] = $_SESSION['cart'][$key]['price'] * $new_quantity;
            $item_found = true;
            break;
        }
    }
...
$response = array(
    'success' => true,
    'message' => $new_quantity === 0 ? 'Item removed from cart' : 'Quantity updated',
    'data' => array(
