<?php
// Product Detail API - Fixed for older PHP versions
...
if ($color_result && $color_result->num_rows > 0) {
    while ($row = $color_result->fetch_assoc()) {
        $color_variants[] = array('id' => intval($row['option_id']), 'name' => $row['display_name'], 'value' => $row['value'], 'image' => !empty($row['color_image']) ? 'https://dentwizard.lgstore.com/pdf/244/' . $row['color_image'] : null, 'price' => floatval($product['Price']));
    }
}

if (empty($color_variants)) {
    $color_variants[] = array('id' => 0, 'name' => 'Default', 'value' => 'Default', 'image' => 'https://dentwizard.lgstore.com/pdf/244/' . $product['ImageFile'], 'price' => floatval($product['Price']));
}
...
if ($formid_result && $formid_result->num_rows > 0) {
    $valid_sizes = array('XS', 'S', 'M', 'L', 'XL', 'XXL', 'XXXL', '2XL', '3XL', '4XL', '5XL', 'LT', 'XLT', '2LT', '3LT', 'MT', 'ST', '2XT', '3XT');
...
if (!empty($product['item_logo_ids'])) {
    ...
    if ($logo_result && $logo_result->num_rows > 0) {
        while ($row = $logo_result->fetch_assoc()) {
            ...
            $logos[] = array('id' => intval($row['ID']), 'name' => $row['Name'], 'image' => $logo_image);
        }
        $has_logos = true;
    }
}

$response = array('status' => 'success', 'data' => array('id' => intval($product['ID']), 'name' => $product['item_title'], 'price' => floatval($product['Price']), 'tax_code' => $product['product_tax_code'], 'image_url' => 'https://dentwizard.lgstore.com/pdf/244/' . $product['ImageFile'], 'image_file' => $product['ImageFile'], 'sku' => $product['FormID'], 'description' => $product['Description'] ? $product['Description'] : 'Quality DentWizard apparel', 'available_sizes' => $sizes, 'color_variants' => $color_variants, 'available_logos' => $logos, 'has_logos' => $has_logos));
