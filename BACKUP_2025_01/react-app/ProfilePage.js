...
function ProfilePage() {
  ...
  const handleProfileUpdate = async () => {
    try {
      ...
      // Update Redux store
      dispatch(setUser({ user: { ...user, ...profileData } }));
    } catch (error) {
  ...
  const handleAddressSubmit = async () => {
    try {
      if (addressDialog.data?.id) {
        await api.put(`/user/addresses/${addressDialog.data.id}`, addressDialog.data);
        setSuccess('Address updated successfully');
      } else {
        await api.post('/user/addresses', addressDialog.data);
                      ...
                      {order.items?.slice(0, 2).map((item, idx) => (
                        <ListItem key={idx} disableGutters>
                          <ListItemText
                            ...
                            secondary={`Qty: ${item.quantity} | Size: ${item.size}`}
                          />
