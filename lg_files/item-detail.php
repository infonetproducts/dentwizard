...
if($item_id==41839)
{
	// this is for cid : 69 , cappyhour
	//header("location:cappyhour.php?id=41839");
	//die;
}
...
{
	// this is for cid :  
	header("location:item-detail-gift.php?id=43866");
	die;
}
...
{
	// this is for cid :  
	header("location:item-detail-gift.php?id=43868");
	die;
}
...
{
	// this is for cid : 69
	header("location:item-detail-gift.php?id=41191");
	die;
}
...
{
	// this is for cid : 143
	header("location:item-detail-gift.php?id=51124");
	die;
}
...
{
	// this is for cid : 118
	header("location:item-detail-gift.php?id=41747");
	die;
}
...
{
	// this is for cid : 62
	header("location:item-detail-gift.php?id=42573");
	die;
}
...
if($item_id==40567)
{
	header("location:item-detail-canvas.php?id=40567");
	die;
}
...
$item_logo_ids_arr = array();

if($item_logo_ids!="")
{
	$item_logo_ids_arr = explode(",",$item_logo_ids);
}
	...
	<div class="eight columns" >
		<div class="slider-padding" >
            ...
            <!-- <div id="product-slider" class="royalSlider rsDefault" align="center">
				<img id="default_img" style="max-height:500px; max-width:500px;"  src="<?php echo $item_image;?>"  />
			 </div>-->
             
			 
			  <?php
				 // include("zoom_img_tag.php");
			 
				//  include("zoom_img_tag_v2.php");
				  
				 //  include("zoom_img_tag_v3.php");
				 
				   include("zoom_img_tag_v4.php");
				   
				  // include("zoom_img_tag_v5.php");
			 ?>
          ...
          <!--   <a class="MagicZoom" data-options="zoomPosition: inner" href="images/1305510white_larger.jpg"><img src="images/1305510white_small.png"></a>
			 ...
			 <input type="hidden" name="is_sale_price" id="is_sale_price"  value="<?php echo $is_apply_sale_price;?>"  >
			
			   <input type="hidden" id="item_img"  value="<?php echo $item_image;?>"  >
...
================================================== -->
	<div class="eight columns">
		<div class="product-page">
			...
			<!-- Headline -->
			<section class="title">
				<h2><?php $item_title = toSafeDisplay_edit_time_shop($item_detail["item_title"]);
				...
				?></h2>
				 ...
				 ?>
				 <span class="product-price-discount">$<span id="ajax_p"><?php echo number_format($item_detail["Price"],2);?></span><i>$<span id="ajax_sale_price"><?php echo number_format($new_price,2);?></span></i></span>
				 </strong></h3> 
				 <?php
				 }else{
				 ?>
				 ...
				 $<span id="ajax_p"><?php echo number_format($item_detail["Price"],2);?></span></strong></h3> 
				<?php
				}
				?>
				...
				if($sale_tax>0)
				{
					//echo "<strong>Sale Tax will apply on this item ".number_format($sale_tax,2)."%</strong>";
				}
			...
			<section>
				...
				<br/>
				<strong> Waist Size (Inches)</strong> 
<input type="text" name="waist_size" id="waist_size" value="" size="50" />

<br/>
				<strong> Length (Inches)</strong> 
<input type="text" name="length_inches" id="length_inches" value="" size="50" />
...
<input type="submit" onclick="myFunction_color(); show_additional_image();" class="dropbtn_color" id="dropbtn_color" value="Please select color" />
  ...
  <div id="myDropdown_color" class="dropdown-content_color">
    <a href="javasript:void(0);" 
	onclick="MagicZoom.switchTo('Zoom-1', 0); select_color('Please select color','<?php echo $first_img;?>');">
	...
	<a   href="javascript:void(0);" onclick="MagicZoom.switchTo('Zoom-1', <?php echo $k_img_color;?>); select_color('<?php echo $k_color;?>','<?php echo $img_tm;?>');"><?php echo $k_color;?></a>
...
<?php
$custom_name_price = $item_detail['custom_name_price'] ; 
$custom_number_price = $item_detail['custom_number_price'] ; 
 $is_add_custom_name = $item_detail['is_add_custom_name'] ; 
$is_add_custom_number = $item_detail['is_add_custom_number'] ; 

$is_name_validation = $item_detail['is_name_validation'] ; 
$is_number_validation   = $item_detail['is_number_validation'] ; 
?>


<?php
if($is_add_custom_name==1)
{
?>

<input type="hidden" name="is_name_validation" id="is_name_validation" value="<?php echo $is_name_validation;?>" />
...
<strong>Custom Name</strong> (Add $<?php echo number_format($custom_name_price,2);?>)
<input type="text" name="custom_name" id="custom_name" value="" size="50" />
<input type="hidden" name="custom_name_price" id="custom_name_price" value="<?php echo number_format($custom_name_price,2);?>"  />
...
<strong>Custom Number </strong> (Add $<?php echo number_format($custom_number_price,2);?>)
<input type="text" name="custom_number" id="custom_number" value="" size="50"  />
<input type="hidden" name="custom_number_price" id="custom_number_price" value="<?php echo number_format($custom_number_price,2);?>"  />
<input type="hidden" name="is_number_validation" id="is_number_validation" value="<?php echo $is_number_validation;?>" />
<?php
}
?>
...
if($item_logo_ids!="")
{
	include("image-picker/logo_drop.php");
...
?>
					...
					<div class="clearfix"></div>
				<?php 
				$is_special_comment = $item_detail['is_special_comment'] ; 
				if($is_special_comment==1)
				{		
				?>

				<strong>Special Comment<br/>
				
                
                
                <textarea name="special_comment" id="special_comment" cols="57" rows="5" ></textarea>
			...
			<section class="linking">
					...
					<form action='#'>
						...
						<?php
					if($item_detail['available_order']=="no")
					{	
					?>
                    
                    <div style="color:red;">&nbsp;&nbsp;
					<?php	echo $item_detail['cus_msg_not_avai'];
...
<?php
				...
				<a href="item-detail.php?id=<?php echo $item_detail['ID'] ; ?>">
				 ...
				 ?>
						 ...
						 <span class="product-price-discount">$<span id=""><?php echo number_format($item_detail["Price"],2);?></span><i>$<span id=""><?php echo number_format($new_price,2);?></span>
...
<?php  include("zoom_include_file.php");?>
